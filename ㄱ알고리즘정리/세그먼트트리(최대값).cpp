/*
세그먼트트리의 변형판으로 구간의 최댓값을 빠르게 계산할 수 있는 방법이다.
*/
...
int find_max(int start, int end, int node, int left, int right){
	...
	if(left > end || right < start) return 0; // -> 구간에 일치하는 부분이 하나도 없는 경우 결과에 영향없게 가장 최솟값을 반환한다 (예시: -INX_MAX)
	if(left <= start && end<=right) return tree[node]; // -> 원하는 구간이 실제 범위로 잡은 구간보다 큰 경우 구간 내 최댓값을 반환한다.
	int mid = (start + end) / 2;
	return max(find_max(start, mid, node*2, left, right), find_max(mid+1, end, node*2+1, left, right)); //재귀로 구간을 탐색하면서 결과를 찾는다.
}
...
int main(){
	...
	find_max(0,9,1,2,5); //2번째에서 5번쨰까지 출력하기 값: 10 ||||| 왜? 첫번째: 0 두번째: 1 로 시작하기 떄문.. 1,2,3,4 => 10
	update_tree(0,9,1,2,10);  // 2번째의 값이 10으로 변동

	cout << find_max(0,9,1,2,2);
	return 0;
}